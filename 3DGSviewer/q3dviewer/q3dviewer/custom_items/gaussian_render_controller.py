...
class GaussianRenderController:
    ...
    def initialize_gl(self, shader_dir):
        try:
            ...
            # therefore degrade to standard viewing instead of losing the
    ...
    def set_mode(self, mode):
        ...
        if mode != "standard" and not self.sphere_available:
            self.mode = "standard"
    ...
    def set_appearance_settings(self, settings):
        """Store a renderer-neutral appearance value for both draw passes."""

        self.appearance_settings = settings
        if self.item.glwidget() is not None:
            self.item.glwidget().update()

    def appearance_uniforms(self):
        settings = self.appearance_settings
        tone_mapping = str(getattr(settings, "tone_mapping", "aces")).lower()
        tone_id = {"none": 0, "reinhard": 1, "aces": 2}.get(tone_mapping, 2)
        return {
            "exposure": float(getattr(settings, "exposure", 0.0)),
            "tone_mapping": tone_id,
            "contrast": float(getattr(settings, "contrast", 1.0)),
            "saturation": float(getattr(settings, "saturation", 1.0)),
            "vignette": float(getattr(settings, "vignette", 0.0)),
        }

    def paint(self):
        if self.mode == "standard" or not self.sphere_available:
        ...
        metrics.update(
            {
                ...
                "sphere_available": self.sphere_available,
                "sphere_error": self.sphere_error,
            }
        )
        return metrics
