...
vec3 computeCov2D(vec4 pc, float focal_x, float focal_y, mat3 cov3D,
                  mat4 viewmatrix, out float aa_factor)
{
	float z2 = pc.z * pc.z;
    mat3 J = mat3(
        focal_x/pc.z,      0.0f,                   0,
	...
	float original_det = max(
		cov[0][0] * cov[1][1] - cov[0][1] * cov[0][1], 0.0f);

	cov[0][0] += 0.3f;
	cov[1][1] += 0.3f;
	float filtered_det = max(
		cov[0][0] * cov[1][1] - cov[0][1] * cov[0][1], 1e-12f);
	aa_factor = sqrt(clamp(original_det / filtered_det, 0.0f, 1.0f));
...
void main() 
{
	...
	float alpha = gs_data[base_gs + OFFSET_DATA_ALPHA] * aa_factor;
