...
sigma 倍数；该显示不会修改原始椭圆或 PLY。交互预览默认使用受控数量的球体，打开
“显示全部外接球”即可检查每一个 Gaussian 的球体实例；模型很大时建议使用高质量
导出查看完整结果。若球体 shader 在当前驱动上不可用，查看器会禁用球体选项并保留
标准 Gaussian 视图。视图面板还可打开地面网格、XYZ 坐标轴、场景包围盒、中心标记
和相机信息叠加；这些辅助线使用独立的 Core Profile 渲染通道，不会复制 Gaussian
数据。

使用 Camera Director 创建漫游时，先移动到视角并点击“添加镜头”，再移动到下一
视角并再次添加；时间轴会自动连接前后视角。播放预览和最终渲染都按固定 FPS 使用
同一套确定性插值。相机面板支持 Orbit/Fly、飞行速度、可编辑的相机位置/目标点和
命名书签；工具栏提供前/后/左/右/上/下标准视角，时间轴可用当前视角更新镜头起点
或终点。窄窗口会在 Inspector 与时间轴之间切换，保证 3D 视口不被挤没。外观
面板的曝光、色调映射、球谐质量（DC 至 SH 3 阶）、对比度、饱和度和暗角会同步到
交互预览与最终输出，背景颜色也可直接编辑。导出支持当前视口、720p、1080p 或
自定义分辨率的 PNG 静帧、PNG 序列和 MP4；输出 FPS 会重新采样同一条时间
轴，确保视频时长和帧率一致。透明背景仅用于 PNG，MP4 优先走 Jetson GStreamer
H.264，必要时回退到 PyAV。导出对话框会显示 Gaussian 数量、估算 GPU 数据、
帧缓冲预算和编码后端；队列有界且不丢帧，取消或编码失败不会发布不完整文件。
