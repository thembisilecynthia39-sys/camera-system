...
Usage:
    QT_QPA_PLATFORM=offscreen python scripts/visualization/widgets/wv_redesign_preview.py
"""
...
def main() -> None:
    ...
    for index, name in enumerate(("正面", "左前", "右前", "顶部")):
        ...
        # Text placeholders keep offscreen Qt's QWidget.grab deterministic;
        # live frame painting is exercised by the normal camera path.
        view._tiles[index]._frame_label.setText(name)
    ...
    # Acceptance-resolution renders for the workstation's supported range.
    for width, height in ((1920, 1080), (1600, 900), (1366, 768)):
        view.resize(width, height)
        _settle(app)
        view.grab().save(str(OUTPUT_DIR / f"redesign_{width}x{height}.png"))
    view.resize(1440, 900)
    _settle(app)
