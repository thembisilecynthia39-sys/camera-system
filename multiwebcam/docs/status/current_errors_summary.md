...
> [!NOTE]
> 本文是 `multiwebcam` 当前测试、硬件验证和剩余风险的状态来源。设备、驱动或测试结果变化时，先更新本文；README 只保留面向用户的稳定说明。

状态内容记录时间：2026-06-26 10:26（Asia/Shanghai）

状态内容最后更新：2026-06-26 10:58（Asia/Shanghai）

文档分类说明更新：2026-07-16（本次未重新验证硬件）
...
- 移除了 `scripts/services/inference_service.py` 中未使用的 `Optional` import。
- 修复了 `scripts/experiments/pyav/02_open_single_device.py` 中 `np.ndarray` 类型引用但未导入 `numpy` 的问题。
- 在 `pyproject.toml` 的 Ruff exclude 中排除了 `multiwebcam_legacy`、`scripts/experiments/pyav` 和 `scripts/visualization/widgets`。

排除策略说明：

- `multiwebcam_legacy/` 是历史实现，当前重写路径集中在 `src/multiwebcam/`。
- `scripts/experiments/pyav/` 是硬件探索脚本，保留实验记录比强制生产级 lint 更有价值。
- `scripts/visualization/widgets/` 为手动 GUI 截图/验证脚本，存在有意的本地路径插入模式，强制 `E402` 会制造噪声。
...
- `scripts/experiments/pyav/`：探索脚本中有未使用 import/变量、f-string 和一个类型注解中的 `np` 未定义问题。
- `scripts/visualization/widgets/`：可视化脚本因为动态插入路径，触发大量 `E402`，同时有未使用变量/import。
- `scripts/services/inference_service.py`：`typing.Optional` 未使用。
- `src/multiwebcam/pipeline/session.py`：`FrameMetadata` import 未使用。

## 已处理的关键 lint 明细

- `scripts/experiments/pyav/02_open_single_device.py:25`：`F821 Undefined name np`，已通过导入 `numpy as np` 并使用真实类型注解解决。
- `scripts/services/inference_service.py:12`：`F401 typing.Optional imported but unused`，已移除。
...
- `scripts/visualization/widgets/*.py`：多处 `E402`，原因是脚本先修改 `sys.path`，再导入 Qt/项目模块。
...
```bash
./.venv/bin/python scripts/jetson/validate_jetson_stack.py --project /home/lab/3DGS/Multcamera/multiwebcam
./.venv/bin/python scripts/diagnostics/diagnose_usb_cameras.py --max-devices 8 --duration 3
./.venv/bin/python scripts/diagnostics/probe_configured_pipeline.py --project /home/lab/3DGS/Multcamera/multiwebcam --duration 5
./.venv/bin/python scripts/jetson/jetson_smoke_pipeline.py --project /home/lab/3DGS/Multcamera/multiwebcam --warmup-seconds 5 --record-seconds 3 --run-inference
```
...
- 重新插拔 USB2.0 摄像头后，项目按新 `bus_info` 自动重绑定：`video2=usb-3610000.xhci-2.1.2`、`video4=usb-3610000.xhci-2.4.3`、`video6=usb-3610000.xhci-2.4.4`。
- 目标分辨率分配已调整为 `video2=640x480`、`video4=1280x720`、`video6=1280x720`；三路 USB2.0 通过项目 `CaptureSession` 连续出帧。
- 固定 USB3.0 `video0=1280x720` 加入后，四路 `CaptureSession` 启动结果为 `active=video0,video2,video4,video6`、`errors={}`、`healthy=True`。
- 真实 Qt 事件循环验证通过，首次摄像头连接由后台 worker 执行；UI 不等待 V4L2 warm-up 才显示窗口。
- 反向分配（`video2/video4=720p`、`video6=640x480`）会由 V4L2 报 `No space left on device`，这是当前 USB2 拓扑的端点调度差异，不是 UI worker 占用。
