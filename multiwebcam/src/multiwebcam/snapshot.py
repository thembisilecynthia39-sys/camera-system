...
def save_snapshot_set(
    ...
    frame_id = _next_frame_id(output_dir / "metadata.csv")
    image_paths: dict[int, Path] = {}

    for device_path, packet in sorted(packets.items(), key=lambda item: camera_info[item[0]].source_id):
        info = camera_info[device_path]
        image_number = _next_image_number(images_dir, image_stem)
        image_path = images_dir / f"{image_stem}{image_number}.jpg"
...
def _next_frame_id(metadata_path: Path) -> int:
    highest = 0
    ...
    with metadata_path.open("r", newline="", encoding="utf-8") as file:
        for row in csv.DictReader(file):
            try:
                highest = max(highest, int(row.get("frame_id", 0)))
            except (TypeError, ValueError):
                continue
    return highest + 1


def _next_image_number(images_dir: Path, capture_name: str) -> int:
    pattern = re.compile(rf"^{re.escape(capture_name)}(\d+)\.jpg$")
    highest = 0
    for path in images_dir.glob(f"{capture_name}*.jpg"):
            ...
            writer.writerow(
                {
                    ...
                    "image_name": image_paths[info.source_id].name,
