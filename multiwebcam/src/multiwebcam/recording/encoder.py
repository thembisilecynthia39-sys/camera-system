...
class CameraEncoder:
        ...
        try:
            ...
            if container is not None and stream is not None:
                try:
                    ...
                    container.close()
                    logger.info(
                        "Encoder for cam_%s finished: %s frames written to %s",
                        self.cam_id,
    ...
    def _initialize_encoder(self, frame_shape: tuple[int, ...]) -> tuple[av.container.OutputContainer, VideoStream]:  # type: ignore[name-defined]
        ...
        stream = container.add_stream(self.settings.codec, rate=self.settings.fps)
        ...
        logger.debug(
            "Initialized encoder for cam_%s: %sx%s, codec=%s",
            self.cam_id,
            width,
            height,
            self.settings.codec,
    ...
    def _run_gstreamer(self) -> None:
        """Record using OpenCV VideoWriter into Jetson's hardware encoder path."""
        writer = None
        try:
            while True:
                try:
                ...
                if writer is None:
                    ...
                    if not writer.isOpened():
                        raise RuntimeError(f"Failed to open GStreamer writer: {pipeline}")

                writer.write(packet.frame)
                self._frames_written += 1
                if self.timestamp_callback is not None:
                    self.timestamp_callback(self.cam_id, packet.frame_index, packet.frame_time)
        except Exception as e:
            error_msg = f"GStreamer encoder failed: {e}"
            logger.error("cam_%s: %s", self.cam_id, error_msg, exc_info=True)
            self._errors.append(error_msg)
            self._drain_queue_on_error()
        finally:
