...
def test_with_defaults():
    ...
    assert profile.capture_backend == "opencv_v4l2"
    assert profile.gstreamer_pipeline is None
...
def test_with_capture_backend():
    profile = SourceProfile.with_defaults(source_id=0, bus_info="usb-1")

    updated = profile.with_capture_backend(
        "gstreamer",
        pipeline="v4l2src device=/dev/video0 ! appsink",
    )

    assert profile.capture_backend == "opencv_v4l2"
    assert updated.capture_backend == "gstreamer"
    assert updated.gstreamer_pipeline == "v4l2src device=/dev/video0 ! appsink"
