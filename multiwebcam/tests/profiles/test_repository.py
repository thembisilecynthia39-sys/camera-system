...
def test_capture_backend_roundtrip(tmp_path):
    repo = ProfileRepository(tmp_path)
    profile = SourceProfile.with_defaults(source_id=0, bus_info="usb-1").with_capture_backend(
        "gstreamer",
    ...
    assert loaded.capture_backend == "gstreamer"
    assert loaded.gstreamer_pipeline == "v4l2src device=/dev/video0 ! appsink"


def test_settings_roundtrip_and_profile_preservation(tmp_path):
    repo = ProfileRepository(tmp_path)
    profile = SourceProfile.with_defaults(source_id=0, bus_info="usb-1")
    repo.save(profile)

    settings = AppSettings(
        recording=RecordingSettings(backend="gstreamer", fps=25, bitrate=4_000_000),
        inference=InferenceSettings(
            ...
            service_script="/tmp/inference_service.py",
        ),
    )
    repo.save_settings(settings)

    loaded_settings = repo.load_settings()
    loaded_profiles = repo.load_all()

    assert loaded_profiles == [profile]
    assert loaded_settings.recording.backend == "gstreamer"
    assert loaded_settings.recording.fps == 25
    assert loaded_settings.inference.backend == "subprocess"
    assert loaded_settings.inference.engine_path == "/models/yolo.engine"
    assert loaded_settings.inference.device == "cuda:0"
    assert loaded_settings.inference.target_class_ids == (0,)
    assert loaded_settings.inference.service_conda_env == "/home/lab/.conda/envs/multiwebcam-jetson-py38"
    assert loaded_settings.inference.service_backend == "ultralytics_tensorrt"
    assert loaded_settings.inference.service_script == "/tmp/inference_service.py"
