...
def test_save_snapshot_set_writes_images_and_metadata(tmp_path):
    ...
    assert result.output_dir.name == "杯子1"
    ...
    assert metadata_rows[0]["image_name"].startswith("杯子")
...
def test_save_snapshot_set_increments_frame_id(tmp_path):
    ...
    first = save_snapshot_set(tmp_path / "captures", "杯子1", "杯子", packets, camera_info, quality)
    second = save_snapshot_set(tmp_path / "captures", "杯子1", "杯子", packets, camera_info, quality)

    assert first.frame_id == 1
    assert second.frame_id == 2
    assert (second.output_dir / "images" / "杯子2.jpg").exists()
