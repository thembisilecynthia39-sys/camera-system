...
- 当前活跃摄像头为 2、4、5；`source_id = 1` 被设置为 `ignore = true`。
- 活跃分辨率组合为 1 路 `640x480` 加 2 路 `1280x720`，像素格式为 MJPEG，采集后端为 `opencv_v4l2`。
...
- 根目录有 `yolo11n.pt`、`yolo11n.onnx`、`yolo11n.engine`，其中当前配置实际使用 `.engine`。
...
- 当前项目三路启用摄像头能采集、推理和录制。
- 端到端 smoke test 通过，输出位于 `/tmp/mwc_jetson_smoke_*`。
- 当前性能不均衡：`/dev/video2` 与 `/dev/video0` 接近 30fps，`/dev/video6` 约 15fps。
