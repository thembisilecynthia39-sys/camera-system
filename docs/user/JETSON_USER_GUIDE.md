...
无法编译球体 shader，球体选项会变灰，查看器会自动回退到标准 Gaussian。视图面板
还提供地面网格、XYZ 坐标轴、包围盒、中心标记和相机信息叠加，便于检查坐标和场景
范围；它们不改变 Gaussian 数据。
...
演示模式。工具栏的“视角”菜单可以快速切换前、后、左、右、上、下标准视角；时间轴
的“更新起点/更新终点”会把当前视角写入选中的镜头段。

Inspector 的“外观”参数会同时作用于交互预览和最终导出；锐化在最终 CPU 后处理中
执行，以保证导出画面可重复。球谐质量可以从 DC 降到 SH 1/2/3 阶，低阶适合 Jetson
交互预览，最终展示建议使用源文件支持的最高阶。

### PNG/MP4 展示输出

在 Inspector 的“渲染”面板选择当前视口、720p、1080p 或自定义分辨率，再设置 FPS、
输出格式和透明背景，最后点击工具栏“导出”：
