...
5. 点击“播放”预览，点击“停止”回到当前时间轴范围的第一帧。

时间轴按固定 FPS 采样，最终输出与预览使用同一套位置、目标点和四元数球面
插值，因此同一项目可以重复生成相同帧数的结果。`Escape` 会退出全屏演示模式。
...
导出时 OpenGL 只在 GUI 线程渲染，编码在后台线程进行；队列满时会等待，不丢帧。
...
显示模式、外接球参数、外观、时间轴和渲染设置，使用 PLY 文件大小与 SHA-256
